//! Transactional, lossless file recovery for semantic and shell emitters.
//!
//! Reservations are committed only when the complete output plus recovery hint fits
//! the emission contract. Legacy failure hints and SQLite recall live in `tee` and
//! `retriever`; these artifacts retain the fork's shell-neutral `read --recovery` API.
...
fn get_tee_dir_with_env(
    ...
    // Config override
    if let Some(ref dir) = config.retriever.tee_directory {
...
pub(crate) fn reserve_lossless_tee_for_emission(
    ...
    if crate::core::retriever::recovery_disabled_by_env() || raw.is_empty() {
    ...
    if config.retriever.mode == crate::core::retriever::RecoveryMode::Disabled {
        return Err(LosslessTeeReservationError::Unavailable);
    }
    let tee_dir = get_tee_dir(&config).ok_or(LosslessTeeReservationError::Unavailable)?;
    let max_files = lossless_tee_max_files_for_test(config.retriever.tee_max_files);
    reserve_lossless_tee_file(
        raw,
        command_slug,
        &tee_dir,
        config.retriever.tee_max_file_size,
        max_files,
    )
    .ok_or(if raw.len() > config.retriever.tee_max_file_size {
...
fn force_tee_path(content: &str, command_slug: &str) -> Option<PathBuf> {
    if crate::core::retriever::recovery_disabled_by_env() {
        return None;
    }
    ...
    write_tee_file(
        ...
        &tee_dir,
        config.retriever.tee_max_file_size,
        config.retriever.tee_max_files,
    )
}
...
"#;
    ...
    fn configured_tee_directory_has_precedence_over_default() {
        ...
        config.retriever.tee_directory = Some(configured.clone());
    ...
    fn test_force_tee_hint_respects_env_disable() {
        ...
        temp_env::with_var("RTK_TEE", Some("0"), || {
