...
impl Tracker {
    ...
    fn get_by_command(
        ...
        let query = format!(
            "SELECT rtk_cmd, COUNT(*), SUM(saved_tokens), CASE WHEN SUM(input_tokens) > 0 THEN SUM(saved_tokens) * 100.0 / SUM(input_tokens) ELSE 0.0 END, AVG(exec_time_ms)
             FROM commands
...
pub fn estimate_tokens_from_len(len: usize) -> usize {
    estimate_tokens_from_bytes(len)
}
...
mod tests {
    ...
    fn test_get_by_command_weighted_savings_rate() {
        ...
        let by_cmd = tracker
            .get_by_command(Some(project), Some(10), false)
    ...
    fn test_weighted_rate_via_get_by_command() {
        ...
        let by_command = tracker
            .get_by_command(None, Some(10), false)
