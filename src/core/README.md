...
### Transactional semantic recovery

`lossless_tee.rs` retains the semantic and Windows-shell emitters' reservation/commit
protocol. It writes complete private `.lossless.log` files only when the candidate
and its recovery hint satisfy the output budget. `rtk read --recovery <id>` reads
these shell-neutral artifacts. The public entry points remain re-exported by `tee`.
These file-backed reservations use `[retriever] tee_*` limits even when legacy hints
use SQLite; `mode = "disabled"`, `RTK_RECALL=0`, and `RTK_TEE=0` disable new recovery.
