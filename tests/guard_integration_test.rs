...
fn git_show_dash_dash_stat_pathspec_after_double_dash_is_not_stat_flag() {
    ...
    assert_eq!(code, Some(0), "rtk stderr: {stderr}");
    // The semantic renderer may retain the native patch when a recovery-bearing summary
    // would be larger. Both forms must retain the path and its addition.
    assert!(
        stdout.contains("--stat")
            && (stdout.contains("+1") || stdout.contains("+not a summary flag")),
        "expected the change for the --stat path: {stdout:?}"
    );
}
...
fn git_diff_dash_dash_stat_pathspec_after_double_dash_is_not_stat_flag() {
    ...
    assert_eq!(code, Some(0), "rtk stderr: {stderr}");
    // Tiny diffs can use the never-worse raw fallback; filename classification must
    // still preserve the actual addition rather than emit only a diffstat.
    assert!(
        stdout.contains("--stat") && (stdout.contains("+line two") || stdout.contains("Changes:")),
        "expected the change for the --stat path: {stdout:?}"
    );
}
